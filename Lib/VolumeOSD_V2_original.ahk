...
global VOLUME_OSD_X		    := A_ScreenWidth/2 - 200
global VOLUME_OSD_Y			:= A_ScreenHeight - 270


VolumeOSD(NewSetting, Period := 3000, Status?, *)           ;  v0.12 by SKAN for ah2 on D48C/D7AR @ autohotkey.com/r?t=93666
{
    global
    ...
    If ( Type(MyGui) = "String" )
    {
        ...
        MyGui.AddText("xm y+m wp  h28 vMyVolume Center 0x200")
    ...
    ;Else IsFloat(NewSetting) 
          ...
          ; : SoundSetMute(NewSetting)
                    ...
                    UpdateProgress(*)
                    {
                        ...
                        MyGui["MyProgress"].Opt("c1d3b40 BackgroundFFFFFF") ; Bright green on Dark green
                    
                        ;Vol := SoundGetVolume()
                        
                        MyGui["MyProgress"].Value := Round(Float(NewSetting))
                        MyGui["MyVolume"].Value   := Round(Float(NewSetting))
                    }                  
...
#SingleInstance
/*
;Status := "Speakers Soundbar"

#F1::VolumeOSD("+1",,    Status?) ; Toggle mute
#F2::VolumeOSD("-2.0",,  Status?) ; Decrease volume
#F3::VolumeOSD("+2.0",,  Status?) ; Increase volume

#F4::VolumeOSD("50.0",,  Status?) ; Half volume
#F5::VolumeOSD("100.0",, Status?) ; Full volume
*/