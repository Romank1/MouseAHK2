...
class winsock {
	...
	MsgSize()
	{
        ...
        argp := 0
		if (DllCall("Ws2_32\ioctlsocket", "UInt", this.desc, "UInt", FIONREAD, "UInt*", &argp) == -1)
	...
	RecvFrom(Flags:=0)
	{
		...
		if ((r := DllCall("Ws2_32\recv", "UInt", this.desc, "Ptr", StrPtr(Buffer), "Int", BufSize, "Int", Flags)) == -1)
			throw Error("Error calling recv",, this.GetLastError())

        return StrGet(StrPtr(Buffer), r, "UTF-8")
	}
